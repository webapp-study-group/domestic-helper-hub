...
import { Company, proxy, DomesticHelper } from '../db/proxy'
import { proxyLib } from '../db/lib'

let { find, filter } = proxyLib
...
export async function collectProfile_HelperPlace(
  ...
  await page.goto(url)
  if (page.url() !== url) {
    // redirected, probably profile page is unlisted
    console.log('removing Helper Place profile:', url)
    let site_id = url.split('/').slice(-2).join('/')
    let row = find(proxy.domestic_helper, {
      company_id: company.id!,
      site_id,
    })
    if (row) {
      delete proxy.domestic_helper[row.id!]
    }
    return
  }
  let _profile = await page.evaluate(url => {
    let profile_pic = document.querySelector<HTMLImageElement>(
      '.profile-preview img',
    )?.src
    if (!profile_pic) {
      throw new Error('failed to find Helper Place profile picture')
    }
    let name = document
    ...
    function findLanguages() {
      for (let h3 of document.querySelectorAll('h3')) {
        if (h3.textContent?.trim() == 'Language:') {
          return Array.from(h3.parentElement!.querySelectorAll('h4'), h4 => {
      ...
      throw new Error('failed to find Helper Place language: ' + url)
    }
    let languages = findLanguages()
    return {
      profile_pic,
      name,
      site_id,
